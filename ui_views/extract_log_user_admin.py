from ui.py.extract_log_user_admin_dialog import Ui_Dialog
...
from db.db_pool import DatabasePool
class ExtractUserLogAdmin(QDialog):
    def __init__(self):
        super().__init__()
        self.ui=Ui_Dialog()
...
QCalendarWidget QToolButton {
    ...
    padding: 15px;
        ...
        self.load_employees()
        self.ui.btn_extract.clicked.connect(self.extract_logs_to_excel)
...
"""
                ...
                usernames=[rec[0] for rec in records]
                self.ui.employee_combo.clear()
                self.ui.employee_combo.addItem("All Employees")
                self.ui.employee_combo.addItems(usernames)
...
"""
    ...
    for row in records
                    ...
                    query="""
select employee_name,login_time,logout_time,username,login_date,day_name,hours_logged from user_logs where login_date between %s and %s and employee_name=%s order by login_date asc



"""
                    cursor.execute(query,(formated_start_date,formated_end_date,employee))
                    records=cursor.fetchall()
                    records=[
                        (
        row[0],  # Employee Name
