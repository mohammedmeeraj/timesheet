from ui.py.assign_project import Ui_Dialog
from PyQt6.QtWidgets import QDialog,QApplication,QCalendarWidget,QMessageBox
...
class AssignUser(QDialog):
    ...
    def __init__(self,project_names,admin_name):
        super().__init__()
        self.ui=Ui_Dialog()
...
QCalendarWidget QHeaderView::section { background-color: #FFFFFF; color: #A0A0A0;    font-size: 12px;}                                
        ...
        self.ui.project_combo.setEditable(True)
        self.ui.project_combo.setCompleter(self.ui.project_combo.completer())
        completer=self.ui.project_combo.completer()
        completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        completer.setFilterMode(Qt.MatchFlag.MatchContains)
        self.ui.project_combo.setMaxVisibleItems(10)
        self.ui.project_combo.currentIndexChanged.connect(self.change_subtask_items)
        self.ui.assign_btn.clicked.connect(self.on_submit)

        
        self.ui.user_combo.mouseDoubleClickEvent=self.delete_user
        self.populate_user_combo_box()
        # self.ui.user_combo.setCompleter(self.ui.user_combo.completer())
        # user_completer=self.ui.user_combo.completer()
        ...
        # self.ui.user_combo.setMaxVisibleItems(10)
    ...
    def delete_user(self,event):
        user=self.ui.user_combo.currentText()
        confirmation=QMessageBox.question(self,"Confirm deletion",f"Are you sure you want to delete user  {user}?",QMessageBox.StandardButton.Yes|QMessageBox.StandardButton.No)
        if confirmation!=QMessageBox.StandardButton.Yes:
            return
        db=GetCursor()
        if db.conn and db.cursor:
            query="delete from users where username=%s and reporting_to=%s"
            db.cursor.execute(query,(user,self.admin_name))
            db.conn.commit()
            if db.cursor.rowcount>0:
                QMessageBox.information(self,"Success","user deleted successfully")
                self.populate_user_combo_box()
            else:
                QMessageBox.warning(self,"Failed","Deletion failed.")
        db.close_connection()

        print(user)
    def populate_user_combo_box(self):
        reporting_to=self.admin_name
        print("reporting tooo",reporting_to)
        try:
            ...
            self.ui.user_combo.addItems(usernames)
            ...
            if conn.is_connected():
                ...
                conn.close()
    def on_submit(self):
        username=self.ui.user_combo.currentText()
        project=self.ui.project_combo.currentText()
        subtask=self.ui.subtask_combo.currentText()
        start_date=self.ui.start_date_le.text()
        end_date=self.ui.end_date_le.text()
        duration=self.ui.duration_le.text()
    ...
    def update_project_names(self,project_names):
        ...
        self.ui.project_combo.addItems(project_names)
    def update_subtask_names(self):
        created_by=self.admin_name
        if self.ui.project_combo.currentIndex==-1:
            pass
        else:
            project_name=self.ui.project_combo.currentText()
            try:
                conn=mysql.connector.connect(
                host="10.95.136.128",
                user="app_user",
                ...
                cursor=conn.cursor()
                query="select subtask from subtasks where project_name=%s and created_by=%s"
                cursor.execute(query,(project_name,created_by))
                n=cursor.fetchall()
                subtask_names=[sub[0] for sub in n]
                self.ui.subtask_combo.clear()
                self.ui.subtask_combo.addItems(subtask_names)
            except mysql.connector.Error as err:
    ...
    def change_subtask_items(self):
        ...
        try:
                ...
                cursor=conn.cursor()
                query="select subtask from subtasks where project_name=%s and created_by=%s "
                cursor.execute(query,(project,created_by))
                n=cursor.fetchall()
                subtask_names=[sub[0] for sub in n]
                self.ui.subtask_combo.clear()
                self.ui.subtask_combo.addItems(subtask_names)
    ...
    def show_start_calender(self):
        self.show_calender(self.ui.start_date_le)
    def show_end_calender(self):
        self.show_calender2(self.ui.end_date_le)
    ...
    def calculate_duration(self):
        start_date_text = self.ui.start_date_le.text()
        end_date_text = self.ui.end_date_le.text()
        if start_date_text and end_date_text:
            ...
            if start_date.isValid() and end_date.isValid():
                ...
                self.ui.duration_le.setText(f"{str(days_difference)} days")
            else:
                self.ui.duration_le.setText("Invalid dates.")
